    ...
    <table class="table divTable table-borderless">
        <thead>
            <tr>
                ...
                <th scope="col" class="spacer">Commission</th>
            </tr>
        </thead>
        <tbody>

            <?php   //print_r($mainarr); die("shifa");?>
            @foreach($mainarr1 as $Valueofarray)
            ...
            <tr>
                ...
                <td class="spacer">{{$Valueofarray['company_name']}}</td>
                <td class="spacer">{{$Valueofarray['invoiceid']}}</td>
                <td class="spacer">${{number_format($Valueofarray['price'],2,'.',',')}}</td>
                 <td class="exspacer"><?php $commission=  $Valueofarray['price']*$Valueofarray['commission']/100?> ${{number_format($commission,2,'.',',')}}</td>
